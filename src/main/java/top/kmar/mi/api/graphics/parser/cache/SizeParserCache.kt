...
class SizeParserCache(exp: String, isHeight: Boolean) : IParserCache {
    ...
    private val task: (Cmpt, (GraphicsStyle) -> Int) -> ISizeMode = exp.removeAllSpace().run {
        // 通过exp解析出一个生成ISideMode的函数
        // 通过此举避免每次生成ISizeMode时都重新解析一遍字符串
        if (startsWith('i', true)) {
            return@run { _, it -> InheritSizeMode { style -> it(style) } }
            ...
            if (mid.endsWith('%'))
                return@run { _, it -> PercentSizeMode(mid.toDouble() * 0.01, 0) { style -> it(style) } }
            ...
            return@run { _, it -> PercentSizeMode(percent, plus) { style -> it(style) } }
        }
    }

    private val setter: (GraphicsStyle) -> Unit = task.run {
        if (isHeight) {
            return@run { it.heightCalculator = this(it.cmpt) { style -> style.height } }
        } else {
            return@run { it.widthCalculator = this(it.cmpt) { style -> style.width } }
        }
    }
