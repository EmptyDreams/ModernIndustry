...
import top.kmar.mi.api.gui.listener.ListenerTrigger;
...
public class StaticFrameClient extends GuiContainer implements IFrame {
	...
	public StaticFrameClient(MIFrame inventorySlotsIn, String title) {
		...
		setResourceName(ModernIndustry.MODID, StringUtil.checkNull(inventorySlotsIn.getID(), "title"));
	}
	...
	public void setTitle(String text) {
		title = StringUtil.checkNull(text, "text");
	}
	...
	public void setTitleModel(TitleModelEnum model) {
		this.titleModel = StringUtil.checkNull(model, "model");
	}
	/** 获取标题显示模式 */
	@Nonnull
	...
	protected final void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		List<IComponent> onComponents = ListenerTrigger.activateEntered(
		...
		while (it.hasNext()) {
			...
			ListenerTrigger.activateExited(inventorySlots, component);
			it.remove();
		}
		...
		ListenerTrigger.activateLocation(inventorySlots, null, mouseX, mouseY);
	...
	private void activeMouseWheelListener(int mouseX, int mouseY) {
		...
		if (wheel == 0) return;
		ListenerTrigger.activateWheel(inventorySlots, null, mouseX, mouseY, wheel);
	}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (mouseButton == 0)
			clickedComponents.addAll(ListenerTrigger.activateAction(inventorySlots, null, fx, fy));
		clickedComponents.addAll(ListenerTrigger.activateClick(inventorySlots, null, fx, fy, mouseButton));
	}
	...
	protected void mouseReleased(int mouseX, int mouseY, int state) {
		...
		for (IComponent component : clickedComponents) {
			ListenerTrigger.activateReleased(inventorySlots, component, mouseX, mouseY, state);
		}
	...
	public void handleKeyboardInput() throws IOException {
		...
		if (Keyboard.getEventKeyState()) {
			...
			for (IComponent it : components) {
				ListenerTrigger.activateKeyPressed(inventorySlots, it, key, clickedComponents.contains(it));
				if (it instanceof IComponentManager) {
					IComponentManager manager = (IComponentManager) it;
					manager.forEachAllComponent(component -> ListenerTrigger.activateKeyPressed(inventorySlots,
			...
			keyCode = -1;
			for (IComponent it : components) {
				ListenerTrigger.activateKeyRelease(inventorySlots, it, key, clickedComponents.contains(it));
				if (it instanceof IComponentManager) {
					IComponentManager manager = (IComponentManager) it;
					manager.forEachAllComponent(component -> ListenerTrigger.activateKeyRelease(inventorySlots,
							component, key, clickedComponents.contains(component)));
				}
			}
		}
