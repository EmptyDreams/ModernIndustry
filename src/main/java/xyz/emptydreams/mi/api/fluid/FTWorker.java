...
public final class FTWorker {
	...
	public static FluidStack applyFluid(World world, BlockPos pos, EnumFacing facing, int amount) {
		if (facing == null) {
			...
			WorldUtil.setBlockState(world, pos, state, transport.getFluid().getBlock().getDefaultState());
			targetTe.markDirty();
			return transport;
		}
	}
	...
	private static FluidStack applyFluidToBlockFromAllFacing(World world, BlockPos pos, int amount) {
		...
		if (result == null || result.amount != 1000) return null;
		WorldUtil.setBlockState(world, pos, state, result.getFluid().getBlock().getDefaultState());
